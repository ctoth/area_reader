...
from area_reader import CoffeeMudAreaFile
...
def main() -> int:
	...
	if missing:
		parser.error("missing CoffeeMud corpus files: %s" % ", ".join(map(str, missing)))
	...
	with tempfile.TemporaryDirectory(prefix="area-reader-coffeemud-") as temporary:
		...
		for relative in CORPUS_PATHS:
			area_file = CoffeeMudAreaFile(upstream / relative)
		...
		baseline_arguments = ["baseline:%s" % relative for relative in CORPUS_PATHS]
		canonical_arguments = [
			"canonical:%s" % (Path("writer-oracle-input") / relative.name)
			for relative in CORPUS_PATHS
		]
		oracle = run(
			[
		...
		for relative in CORPUS_PATHS:
			...
			print(
				"%s: baseline=%s records/%s errors; canonical=%s records/%s errors"
				% (name, baseline_count, baseline_errors, canonical_count, canonical_errors),
			)
			if baseline_count is None or canonical_count is None:
				print("CoffeeMud did not report both loader results for %s." % name)
				return 1
			if canonical_count != baseline_count:
				print("CoffeeMud loaded a different record count for %s." % name)
				return 1
			if canonical_errors is None or baseline_errors is None:
				print("CoffeeMud did not report both diagnostic counts for %s." % name)
				return 1
			if canonical_errors > baseline_errors:
				print("Canonical %s introduced CoffeeMud loader diagnostics." % name)
