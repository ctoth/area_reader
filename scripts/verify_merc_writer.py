...
from area_reader import MercAreaFile
...
def main() -> int:
	...
	area_file = MercAreaFile(source_area)
	...
	with tempfile.TemporaryDirectory(prefix="area-reader-merc-") as temporary:
		...
		if not target.exists():
			raise SystemExit(
				"The source basename must already be listed by the upstream Merc tree: %s"
				% source_area.name
			)
	...
	print("Merc accepted %s and remained live through the boot window." % source_area)
	return 0
