...
from area_reader import SmaugAreaFile
...
def main() -> int:
	...
	area_file = SmaugAreaFile(source_area)
	...
	with tempfile.TemporaryDirectory(prefix="area-reader-smaug-") as temporary:
		...
		with (runtime / "area" / "area.lst").open(
			"w", encoding="latin-1", newline="\n"
		) as area_list:
			area_list.write("help.are\ngods.are\nlimbo.are\n%s\n$\n" % source_area.name)
		...
		header_text = header_text.replace(
			'#define RUNDIR "%s/install/com/smaug/"' % wsl_root,
			'#define RUNDIR "%s"' % wsl_runtime,
		)
		...
		if source_area.name not in boot_output:
			print("The engine stayed live but did not report loading %s." % source_area.name)
			return 1

	print("SMAUG accepted %s and remained live through the boot window." % source_area)
	return 0
