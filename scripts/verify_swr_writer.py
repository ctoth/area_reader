...
from area_reader import SwrAreaFile
...
def main() -> int:
	...
	area_file = SwrAreaFile(source_area)
	...
	with tempfile.TemporaryDirectory(prefix="area-reader-swr-") as temporary:
		...
		if not target.exists():
			raise SystemExit(
				"The source basename must already be listed by the upstream SWR tree: %s"
				% source_area.name
			)
		...
		if source_area.name not in boot_output:
			print("The engine did not report loading %s." % source_area.name)
			return 1
	...
	print("SWR accepted %s and remained live through the boot window." % source_area)
	return 0
