...
def load_circle(root: Path) -> area_reader.CircleAreaFile:
	area_file = area_reader.CircleAreaFile(root)
...
def test_circle_write_uses_the_canonical_tree(tmp_path: Path) -> None:
	...
	for relative_path, text in source.dumps().items():
		assert (output / "lib" / "world" / relative_path).read_text(
			encoding="latin-1"
		) == text
...
def test_circle_native_loss_ledger_has_executable_comment_witness(tmp_path: Path) -> None:
	assert area_reader.CircleAreaFile.NATIVE_NORMALIZATIONS == (
		"comments",
...
@pytest.mark.parametrize(
	"exit_info",
	(
		area_reader.EXIT_FLAGS.PICKPROOF,
		area_reader.EXIT_FLAGS.ISDOOR | area_reader.EXIT_FLAGS.NOPASS,
	),
)
