...
def int_flag_types():
    return [enum_type for enum_type in enum_types() if issubclass(enum_type, enum.IntFlag)]
...
@pytest.mark.parametrize(
    ("enum_type", "member_name", "member"),
    ENUM_MEMBERS,
    ids=[f"{enum_type.__name__}.{member_name}" for enum_type, member_name, _ in ENUM_MEMBERS],
)
...
@pytest.mark.parametrize(
    ...
    ATTRS_ENUM_FIELDS,
    ids=[f"{owner.__name__}.{attribute.name}" for owner, attribute, _ in ATTRS_ENUM_FIELDS],
)
def test_every_enum_typed_attrs_field_jsonifies_recursively(
    ...
    assert result[attribute.name] == (f"{enum_type.__name__}.{member.name}")
    assert json.loads(json.dumps(result)) == result
