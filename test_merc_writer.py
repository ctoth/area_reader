...
def load_merc(path: Path) -> area_reader.dialects.merc.MercAreaFile:
    area_file = area_reader.dialects.merc.MercAreaFile(path)
    area_file.load_sections()
    return area_file


def parse_rendered_merc(tmp_path: Path, text: str) -> area_reader.dialects.merc.MercAreaFile:
...
def test_merc_exit_rejects_rom_only_lock_states() -> None:
    exit = area_reader.dialects.merc.MercExit(
        door=0,
        exit_info=constants.EXIT_FLAGS.ISDOOR | constants.EXIT_FLAGS.NOPASS,
    )
